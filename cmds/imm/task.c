...
room=zuji(dir2[1])+where->query("short");
    if (where) str += sprintf(HIM"%-26s",room);
        str += sprintf(WHT"%-10s  ",ob->name());
if (wizardp(me)) str += sprintf(HIR"%-8s %-8s %-8s %-8s\n"NOR, ob->query_temp("nhdjoblb"),ob->query_temp("nhdjoben"), ob->query_temp("nhdjobtg"), ob->query_temp("nhdjobsc"));
if (!wizardp(me))  str += sprintf(HIR"%-8s \n"NOR, ob->query_temp("nhdjoblb"));
    }
    str += HIG"─────────────────────────────────────\n"NOR;
    str = sprintf("%s共 %d 个随机任务。\n", str, j);
if (me->query_temp("nhdjob2lb"))
{
    str += sprintf("你现在的任务是:%s。\n", me->query_temp("nhdjob2sc"));
if (wizardp(me)) str += sprintf("任务情况:%s %s。\n", me->query_temp("nhdjob2lb"),me->query_temp("nhdjob2tg"));
}
    me->start_more(str);
    me->start_busy(3);
    return 1;
