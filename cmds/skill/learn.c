...
int main(object me, string arg)
{
	...
	object ob;
	int master_skill, my_skill, jing_cost, times, pertimes,intx,i;
string *args;
        if (me->is_busy())
	...
	if (times < 1 || times > 600)
		return notify_fail("学习次数最少一次，最多也不能超过三百次。\n");
...
if (me->query_temp("learn_timem") <= 0) me->set_temp("learn_timem",0);
...
if ((!me->query("zhuanshen") && !me->query("guard/flag") && !me->query("zhuanbest") && !me->query("4zhuan") && !me->query("5zhuan")) || me->query("hyvip")==4) intx=intx*2+1;
...
if( (int)me->query("jing") > jing_cost * times || (!me->query("zhuanshen") && me->query("combat_exp",1) < 12000000))
	{
		...
		&& my_skill * my_skill * my_skill / 10 > (int)me->query("combat_exp") )
		{
			tell_object(ob, "也许是缺乏经验，你对"+ob->name()+"的回答可能无法领会。\n");
			//return notify_fail("也许是缺乏经验，你对"+ob->name()+"的回答总是无法领会。\n");
		} 
