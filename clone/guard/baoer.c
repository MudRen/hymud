...
return notify_fail("ÄãÕıÔÚ×öÈÎÎñ!\n");
if (!arg)
...
³µÕ¾(dache) ÑïÖİÂí¾Ç(majiu) ÑïÖİµ±ÆÌ(dp) ÎäÃí(wm) ÈÕ±¾(jp) \n"NOR, me);
...
return notify_fail("ËûÕıÃ¦×ÅÄØ£¡\n");
        ...
        if (! target || ! this_object()->is_fighting(target))
                return notify_fail("ºÏ»÷Ö»ÄÜ¶ÔÁ½ÈË¶¼ÔÚÕ½¶·ÖĞµÄµĞÈËÊ¹ÓÃ¡£\n");
...
{
message_vision(HIC"$NµÄÉËºÃÏñºÃÁËºÜ¶à£¡£¡\n" NOR,me,target);
if (me->query("qi") < me->query("max_qi"))
