...
#ifdef DEBUG
                        CHANNEL_D->channel_broadcast("debug", "成功解析 "+code+stocks[code]["股票名称"]+" 个股资料(现价 "+stocks[code]["现价"]+")");
#endif
                }
        }
...
void get_stock_data()
{
        ...
        if( fd < 0 )
        {
                CHANNEL_D->channel_broadcast("debug", "无法与新浪股票服务器建立连接。\n");
                return;
        }
...
void write_callback(int fd)
{
        ...
        //CHANNEL_D->channel_broadcast("debug", "抓取"+connection["array"][0]+"资料。");
