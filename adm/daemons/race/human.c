...
//                }
        ...
        if( userp(ob) || undefinedp(my["max_qi"]) )
        {
                ...
                //kar作用
                my["max_qi"] += ob->query("kar") * 10;
                //per作用
                my["max_qi"] += ob->query("per") * 50;
                
                //kar作用
                my["max_jing"] += ob->query("kar") * 10;
                //per作用
                my["max_jing"] += ob->query("per") * 50;
                ...
                // con：
                my["max_qi"] += ob->query("con");
                my["max_jing"] += ob->query("con");
                
                if((int)ob->query_skill("force", 1) >= 10)
                {

                        my["max_qi"] +=    (int)ob->query("con") * (int)(ob->query_skill("force", 1)/5);
                        my["max_jing"] +=  (int)ob->query("con") * (int)(ob->query_skill("force", 1)/5);
                }
  ...
  if(((int)ob->query_skill("chengzi18po", 1)) >= 100)
                {
                        ...
                        my["max_qi"] += ((int)ob->query_skill("chengzi18po", 1)*5);
                }

  if(((int)ob->query_skill("qingzi9da", 1)) >= 100)
                {

                        my["max_jing"] += ((int)ob->query_skill("qingzi9da", 1)*5);
                }

  if(((int)ob->query_skill("dramaturgy", 1)) >= 100)
                {

                        my["max_jing"] += ((int)ob->query_skill("dramaturgy", 1)*5);
                }

  if(((int)ob->query_skill("goplaying", 1)) >= 100)
                {

                        my["max_jing"] += ((int)ob->query_skill("goplaying", 1)*5);
                }
 
  if(((int)ob->query_skill("horticulture", 1)) >= 100)
                {

                        my["max_qi"] += ((int)ob->query_skill("horticulture", 1)*5);
                }

  if(((int)ob->query_skill("luteplaying", 1)) >= 100)
                {

                        my["max_qi"] += ((int)ob->query_skill("luteplaying", 1)*5);
                }

  if(((int)ob->query_skill("medicine", 1)) >= 100)
                {

                        my["max_qi"] += ((int)ob->query_skill("medicine", 1)*5);
                }
  if(((int)ob->query_skill("miaoshouhuichun", 1)) >= 100)
                {

                        my["max_jing"] += ((int)ob->query_skill("miaoshouhuichun", 1)*5);
                }
  ...
  if(((int)ob->query_skill("painting", 1)) >= 100)
                {
                        ...
                        my["max_jing"] += ((int)ob->query_skill("painting", 1)*5);
                }
  if(((int)ob->query_skill("construction", 1)) >= 100)
                {

                        my["max_jing"] += ((int)ob->query_skill("construction", 1)*5);
                }

  if(((int)ob->query_skill("martial-cognize", 1)) >= 100)
                {

                        my["max_jing"] += ((int)ob->query_skill("martial-cognize", 1)*5);
                        my["max_qi"] += ((int)ob->query_skill("martial-cognize", 1)*5);
                }
...
if (!anat) anat=50;
if (anat<=0) anat=200;
if (anat<200) anat=200;
if (anat>2800) anat=2800;
if ((int)ob->query_temp("ana_t/jing")>=1)
{
		my["max_jing"]+=(int)((anat*2) /50 * ((int)ob->query_temp("ana_t/jing")*8));
//		my["eff_jing"]+=(int)(anat*2) /50 *((int)ob->query_temp("ana_t/jing")*2));
}
if ((int)ob->query_temp("ana_t/qi")>=1 && (int)ob->query_skill("force",1)>1)
{
		my["max_qi"]+=(int)((anat*2) /50 *((int)ob->query_temp("ana_t/qi")*8));
...
if ( userp(ob) && ob->query("zhuanshen"))
{
my["max_qi"] += 2000;
my["max_jing"] += 2000;
}

if (userp(ob) && ob->query("zhuanbest"))
{
my["max_qi"] += ob->query("per") * 50;	
my["max_qi"] += ob->query("con") * 50;
my["max_jing"] += ob->query("per") * 30;	
my["max_jing"] += ob->query("con") * 30;
my["max_qi"] += 3000;
my["max_jing"] += 3000;
}

if (userp(ob) && ob->query("4zhuan"))
{
my["max_qi"] += 5000;
my["max_jing"] += 5000;
}
if (userp(ob) && ob->query("5zhuan"))
{
my["max_qi"] += 5000;
my["max_jing"] += 5000;
}
