...
int do_heal(string arg)
{
	...
	if( ! this_object()->query("equipped"))
      ...
      if( me->query("combat_exp", 1) >=2000000 )
		return notify_fail("你的经验太高用不了这个银针！\n");
...
//这一段是为了防止 妙手回春 成为超级 perform
	if(!userp(ob))
	{
                if( me->query_skill("miaoshouhuichun", 1)<10 )
		{
			message_vision(
...
$n不想让$N给他刺穴疗伤。\n",me,ob);
	...
	{
		if( userp(me) && ob!=me && ob->query("env/agree_heal")!=1 )		
		{
...
//        if(me==ob) return notify_fail("你不能向自己用银针，手够不着啊！\n");
	...
	tell_object(me,"在实践中你的妙手回春水平上升了。\n");
             ...
             me->add("score",score);
             me->add_temp("nbjob41",1);
             if (random(2)==0) me->add_temp("yinzhen", 1);
...
//      if(me==ob) return notify_fail("你不能向自己用银针，手够不着啊！\n");
	...
	else
	{
		...
		heals= 10+(me->query_skill("miaoshouhuichun", 1)*2)+ random(me->query_skill("miaoshouhuichun", 1)) ;
                if((ob->query("eff_qi")+heals) > ob->query("max_qi"))
                        ob->set("eff_qi",ob->query("max_qi"));
		else
                        ob->add("eff_qi",heals);

		if((ob->query("eff_jing")+heals) > ob->query("max_jing"))
			ob->set("eff_jing",ob->query("max_jing"));
		else
			ob->add("eff_jing",heals);
