...
int do_save(string arg)
{
        ...
        me=this_player();
        if(strsrch(environment(me)->query("short"),"当铺")<0)
            return notify_fail("你只能在当铺存物品。\n");
      ...
      if(amount<1)
		...
		card->add("rich",sub);
		card->save();
        environment(me)->add("rich",sub);
        printf("当铺老板说道：\n");printf("在乾坤袋中存东西需收税四分之一！\n");printf("客官用不着您费心, 我自己从您兜里拿就成了。\n");
