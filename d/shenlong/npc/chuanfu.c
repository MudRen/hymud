...
//inherit F_SKILL;
int  chu_hai();
void goto_dao(object ob,object obj);
...
obj=present(ob->query("id")+" "+"wulin renwu", environment(ob));
 if (obj) obj->move ("/d/shenlong/dahai");        

        ob->move ("/d/shenlong/dahai");
        tell_object(ob, BLU "你在海上航行了很久很久.......\n" NOR ) ;
	call_out("goto_shenlong",10,ob,obj) ;
       	return 1;
  }
  else  
...
int accept_object(object who, object ob)
{
	...
	obj=present(who->query("id")+" "+"wulin renwu", environment(who));
        if (obj) obj->move ("/d/shenlong/dahai");
	...
	call_out("goto_shenlong",10,who,obj) ;
       	return 1;
	}
    else  
...
void goto_shenlong (object ob,object obj)
{
   tell_object(ob , "大船终于停在了一个小岛边。你走下船来。\n" ) ;
   ob->move ("/d/shenlong/haitan") ;
if (obj) obj->move("/d/shenlong/haitan");
}
