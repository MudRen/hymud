...
#define KICK_TO	__DIR__"center_e"
...
void quarter_pass()
{
	...
	if (local[2] < 5) kick_all();
	else if (local[2] < 11) set_temp("serve", "早");
	//else if (local[2] < 11) set_temp("serve", "无");
	else if (local[2] < 17) set_temp("serve", "午");
	//else if (local[2] < 17) set_temp("serve", "无");
	else if (local[2] < 23) set_temp("serve", "晚");
	else kick_all();
	ob = all_inventory(this_object());
	for(i = 0; i < sizeof(ob); i++) {
