...
string *MjC = ({"",
                ...
                "Ò»Ë÷","¶şË÷","ÈşË÷","ËÄË÷","ÎéË÷","ÁùË÷","ÆßË÷","°ËË÷","¾ÅË÷",
...
varargs int do_win(string str,object me)
{
        ...
        if (do_check_win(sort_data(who["Mj"]))==1||i==1){//ÎªÁË·ûºÏÓÒ±ß·ÅÅÆµÄÏ°¹ß¡­¡­
//ËãÌ¨///////////////////////////////////
        mixed MjT = ([
...
int do_touch(string str,object me)
{
        ...
        if (who["Show"]=="´úÂë") i=3;
        if (who["Show"]=="color") i=5;
        if(userp(me)) tell_object(me,"»¹ÓĞ"+chinese_number((end-count-16-Goned)/2)+"Ö»ÅÆ¾ÍÁ÷¾ÖÁË¡£\n"+show_mj(who["Mj"],i));
...
int do_look(string arg)
{
        ...
        if (arg=="1p"||arg=="2p"||arg=="3p"||arg=="4p"){
           ...
           Mj+="ÄãµÄÉè¶¨ÓĞ:";
              ...
              if (who["Show"]=="´úÂë") i=3;
              if (who["Show"]=="color") i=5;              
              tell_object(me, "ÏÖÔÚÊÇ["+TIMES+"] :×¯¼Ò"+HIY+NowPlayerId[0]+NOR+"\n" );
              Mj+=HIG+"¶«¼Ò  "+NOR+P1Data["Id"]+":°ÚÔÚÍâÃæµÄÅÆÓĞ:\n";              
              Mj+=show_mj(next_data(P1Data["Id"], 0, "allout"),i+1);              
              Mj+=HIG+"ÄÏ¼Ò  "+NOR+P2Data["Id"]+":°ÚÔÚÍâÃæµÄÅÆÓĞ:\n";
              Mj+=show_mj(next_data(P2Data["Id"], 0, "allout"),i+1);             
              Mj+=HIG+"Î÷¼Ò  "+NOR+P3Data["Id"]+":°ÚÔÚÍâÃæµÄÅÆÓĞ:\n";
              Mj+=show_mj(next_data(P3Data["Id"], 0, "allout"),i+1);                                         
              Mj+=HIG+"±±¼Ò  "+NOR+P4Data["Id"]+":°ÚÔÚÍâÃæµÄÅÆÓĞ:\n";
...
write(@HELP
    ...
    ©¦   continue     ´ßNPC»òÍæ¼Ò³öÅÆ£¬ÌØÊâÇé¿öÊÇÌØ±ğÓĞÓÃ¡£     ©¦
...
int get_tc(mapping who)
{
        ...
        if (tempB==""&&check==0&&who["OutPon"]==""&&K==0) Tc+="@d";//Æ½ºúµÄÒªÇóÃ»ÄÇÃ´¿Á¿Ì°É£¿£¿
        //ºÓµ×ÃşÔÂ
...
//´ÓÕâÒÔÏÂÊÇ»ìÒ»É«
	...
	//tell_object( present("wiman"), "»ìÒ»É«´ò·¨ ");
	who["Hu_type"] = "hunyise";
	if( sizeof(tempW)>10 ) {		
        ...
        if( sizeof(tempT)>10 ) {
        	//tell_object( present("wiman"), "»ìT´ò·¨ ");
        	if (tempW!="") filt_mj(tempW, me);
        	if (tempS!="") filt_mj(tempS, me);
        	if (tempS=="" && tempW=="") {
	...
	//for(i=0; i<3; i++) {
        	...
        	if( !userp(ob) && who["Hu_type"]!="pinhu") {
        		...
        		if( do_check_Mj(next_data(me->query("id"), i+1, "Mj"), LastDump)>2 ){
        			...
        			PAUSE = 2;	//°ÑÊ±¼äÅªÍ££¬·ÀÖ¹Íæ¼Ò¶ÔÕ½Ê±ÇÀÃş¡­¡­
        			tell_object(ob, "ÊÇ·ñºú»òÅö»ò¸Ü"+ do_check_num(LastDump, 0)+"£¿£¿£¨Èç¹ûÊÇ£¬ÇëÊäÈë£ºpon/gon£©\nÈç¹û·ñ£¬ÇëÊäÈëcontinue\t");
        			who["Pause"] = "yes";        			
        			return 1;
        		}
...
int do_continue(object me)
{
	...
	tell_object( me, NowPlayerId[NowPlayer]+"-->µ±Ç°´òÅÆÕß¡£\n");	
