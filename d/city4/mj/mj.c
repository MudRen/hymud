...
set_name("°×¾§¾§",({"bai jingjing", "bai", "jingjing", "jing"}));
        ...
        set("age", 18);
        set("long",
                "Ëý¾ÍÊÇ°×¾§¾§£¬Äã¿ÉÒÔÎÊËý¹ØÓÚÂé½«µÄÎÊÌâ\n"
                );
...
int do_gon(string str,object me)
{
        ...
        if (i < 3) return notify_fail("ÄãÊÖÖÐÒªÓÐÈþÕÅ"+do_check_num(str,0)+"²ÅÄÜ¸Ü¡£\n");
        if (!Touched){
        ...
        if (Touched && NowPlayerId[NowPlayer]==me->query("id")){
           NO_HU=0;
           message_vision("\n$NÁ³ÉÏÂ¶³öÐ°¶ñµÄÐ¦ÈÝËµµÀ£º°µ¸ÜºÙºÙ.....\n\n",me);
           who["Mj"]=do_delete_Mj(who["Mj"],str,1);
...
varargs int do_win(string str,object me)
{
        ...
        if (LastDump==""&&Touched) message_vision("\n$N´ó½Ð£º"+HIW+"Ìì¡«¡«¡«¹ÅÔÂ¡«¡«¡«"+NOR+"\n\n",me);
        else{
             tell_room(environment(this_object()),"\n"+me->name()+"´ó½Ð£º"+HIW+"ÎÒ¡«¹ÅÔÂ¡«À²¡«"+NOR+"\n\n",me);
             tell_object(me,"\nÄã´ó½Ð£º"+HIW+"ÎÒ¡«¹ÅÔÂ¡«À²¡«"+NOR+"\n\n");
        }
        if (Touched&&NowPlayerId[NowPlayer]==me->query("id"))
...
int do_pon(string str,object me)
{
        ...
        message_vision("\n$N´ó½Ð£º"+do_check_num(str,0)+"¡«"+HIW+"Ê¯K¡«¡«¡«"+NOR+"\n\n",me);
        for (i=0;i<Play;i++) {
            if (NowPlayerId[NowPlayer]==me->query("id")) break;
            NowPlayer++;
            NowPlayer%=Play;
