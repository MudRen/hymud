...
void greeting(object ob)
{
	if( !ob || environment(ob) != environment() ) return;
			...
			say( HIG"\n卖  "+HIR+"sell <物品ID>\n"NOR);
			say( HIG"看  "+HIR+"list \n"NOR);
			say( HIG"买  "+HIR+"buy <物品ID>\n"NOR);
			say( HIG"赎  "+HIR+"redeem  <物品ID>\n"NOR);
			say( HIG"估价  "+HIR+"value <物品ID>\n"NOR);
...
//		if( target == (string)all[i]->id() || target == all[i]->name() )
	...
	if( !dest )
		return notify_fail("唐古说到：他现在不在游戏中，过会再来吧。\n");
	else if( !dest->query_temp("selled"))
		return notify_fail("唐古急道：你可别诬陷好人，我怎么会非法扣押玩家呢！\n");
