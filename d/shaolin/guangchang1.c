...
int valid_leave(object me, string dir)
{
   ...
   if(me->query("jinyong/book4")==8 && !query("iskillok1"))
      {
      message_vision("丁春秋从边上杀了出来 说道：我就领教领教你的武功吧!\n",who);	
      	fang = load_object("/d/mr/npc/ding");
	fang->move(environment(who));	
	fang->kill_ob(who);
      	message_vision("鸠摩智从边上杀了出来 说道：让你看看我的小无相功!\n",who);	
      	fang = load_object("/d/mr/npc/jiumozhi");
	fang->move(environment(who));	
	fang->kill_ob(who);
     this_object()->set("iskillok1",1);
     set("iskillok1",1);
return notify_fail("先把这两个老怪物消灭了吧。\n");
      }	
...
if (!userp(me))
    ...
    if ( (userp(me) && me->query("family/family_name") == "少林派"))
    {
