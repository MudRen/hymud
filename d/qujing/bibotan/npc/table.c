...
{	
        ...
        if( (int)who->query("qi") < 100
        ||      (int)who->query("neili") < 100
        || ((int)who->query("str") + (int)who->query("con") ) < 60 ) {

		message_vision("$N试着去推石桌，可石桌却象扎了根一般动也不动。\n",who);
                return 1;
        }
        who->receive_damage("qi", 80);
        who->add("force", -30);
