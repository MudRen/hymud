...
int do_chi(string arg)
{
    ...
    m_sen = (int)this_player()->query("max_jing");
	heal = (int)this_player()->query_con();
    e_sen = (int)this_player()->query("eff_jing");
	
    if ( e_sen < m_sen )
	{ 	
		if ( (e_sen + heal) >= m_sen )
		{
			this_player()->set("eff_jing", m_sen);
		} else
		{	
    ...
    e_sen = (int)this_player()->query("eff_jing");
    sen = (int)this_player()->query("jing");
	recover = 30+heal;
