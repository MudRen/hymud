...
#include <ansi.h>
#define CHAMPION_FILE "/data/npc/champion_yd.o"
inherit NPC;
inherit F_MASTER;
...
void changeshape( object winner)
{
	...
	if( (string)winner->query("family/family_name") != "雁荡派" ) 
	return notify_fail("只有雁荡派弟子才能争夺大弟子之位！\n");


	winner_file = winner->query_save_file();
        CHANNEL_D->do_channel(this_object(), "party",
        sprintf( "%s从现在开始是雁荡派新的掌门大弟子了！！ \n",
        winner->name(1)));
	...
	set("title", HIY "雁荡派掌门大弟子" NOR);	
	set("nickname",nick);
	zheng = new(__DIR__"ling_yd");
	find(id);
	zheng->move(winner);
	message_vision("掌门大弟子给了$N一块雁荡派的令牌。\n",winner);
	 set_temp("no_kill",1);set("pubmaster",1);
...
object ob;
ob=this_object();
	if( (string)this_player()->query("family/family_name") != "雁荡派" ) 
	return notify_fail("只有雁荡派弟子才能争夺大弟子之位！\n");
...
ulist  =  users();
	i  =  sizeof(ulist);
while (i--) {
	if  (present  ("ydling",ulist[i]))
	{
	ilist  =  all_inventory(ulist[i]);
	j=sizeof(ilist);
	while (j--) {
	ob = ilist[j];
	if(ob->query("id")=="ydling" && !userp(ob))
	{
	destruct(ob);
			}
	}
message_vision(HIY"$N身上的雁荡派令牌不翼而飞了。\n"NOR,ulist[i]);
	}
}
}
