...
#include <ansi.h>
#define CHAMPION_FILE "/data/npc/champion_xkd.o"
inherit NPC;
inherit F_MASTER;
...
void changeshape( object winner)
{
	...
	if( (string)winner->query("family/family_name") != "侠客岛" ) 
	return notify_fail("只有侠客岛弟子才能争夺大弟子之位！\n");

	winner_file = winner->query_save_file();
        CHANNEL_D->do_channel(this_object(), "party",
        sprintf( "%s从现在开始是侠客岛新的掌门大弟子了！！ \n",
        winner->name(1)));
	...
	set("title", HIY "侠客岛掌门大弟子" NOR);	
	set("nickname",nick);
	zheng = new(__DIR__"ling_xkd");
	find(id);
	zheng->move(winner);
	message_vision("掌门大弟子给了$N一块侠客岛的令牌。\n",winner);
	 set_temp("no_kill",1);set("pubmaster",1);
...
object ob;
ob=this_object();
	if( (string)this_player()->query("family/family_name") != "侠客岛" ) 
...
ulist  =  users();
	i  =  sizeof(ulist);
while (i--) {
	if  (present  ("xiakeling",ulist[i]))
	{
	ilist  =  all_inventory(ulist[i]);
	j=sizeof(ilist);
	while (j--) {
	ob = ilist[j];
	if(ob->query("id")=="xiakeling" && !userp(ob))
	{
	destruct(ob);
			}
	}
