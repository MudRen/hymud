...
int do_sell(string arg)
{
  ...
  if(environment(cook)!=find_object("/d/xiyou/wudidong/kitchen.c"))
  ...
  baoziexp=ob->query("combat_exp")+ob->query("combat_exp");// baoziexp 是被卖者的强度。
...
//  me->add("combat_exp", reward);
  ...
  if(pot+pot_reward<=200)
    me->add("potential", random(100)+10);
  ...
  if (userp(ob))
  {
      ...
      ob->move("/d/xiyou/wudidong/steamer");
...
int check_ratio(object me, object ob)
{
    ...
    baoziexp=(ob->query("combat_exp")+ob->query("combat_exp"))/2;// baoziexp 是被卖者的强度。
    myexp=me->query("combat_exp")+me->query("combat_exp");// myexp 是卖者的强度。
...
{ 
    int base_reward;
    int baoziexp=baozi->query("combat_exp")+baozi->query("combat_exp");
    int adjust=ratio/10;
...
int do_tellme(string arg)
{
    ...
    msg=GRN+seller->query("name")+"（经验："+(seller->query("combat_exp")+seller->query("combat_exp"))+"），";
    msg+="卖"+baozi->query("name")+"（经验："+(baozi->query("combat_exp")+baozi->query("combat_exp"))+"）。\n";
