...
"  基本内功 (force)                         - 一代宗师  290/    \n"+
"  基本招架 (parry)                         - 一代宗师  290/    \n"+
"  基本拳脚 (unarmed)                       - 一代宗师  290/    \n"+
"  基本鞭法 (whip)                          - 深不可测 350/    \n"+
"□秋风尘法 (qiufeng-chenfa)                - 深不可测 350/    \n"+
"□神行百变 (shenxing-baibian)              - 深不可测 350/    \n");
	return 1;
}
...
void greeting(object ob)
{
  ...
  object me=ob;
	...
	if((int)ob->query_temp("hastrystab")==6)
	{
     ...
     me->set_skill("shenxing-baibian",200);
...
who=me;
if (!who->query("jinyong/book7") && who->query_temp("book71"))
{
	i = 300+random(800);
	who->set("jinyong/book7",1);
	who->add("combat_exp",i+5880);
	who->add("potential",i*10);
	who->add("mpgx",10);
	who->add("expmax",2);

	tell_object (who,"你赢得了"+chinese_number(i+5880)+"点经验"+
	           chinese_number(i*10)+"点潜能"+"十点门派贡献 二点成长上限！！\n");
	who->save();
}

		message_vision(HIC "白衣女尼对$N微微一笑，听说陈近南是个大英雄 真想见见他。\n" NOR,ob);		
		return;
	}
