...
int ask_test2()
{
        ...
        {
        command("say Õâ¹ØÄã»¹ÒªºÍÅ·ÑôÊÀÖ¶±ÈÒ»ÏÂÎä¹¦¡£");
...
return 1;
        ...
        {
        command("say Äã´ò°ÜÅ·ÑôÊÀÖ¶ÁËÂğ¡£");
return 1;
}
...
int ask_test3()
{
	object fang;
        ...
        {
        command("say Õâ¹ØÄã»¹ÒªºÍÅ·ÑôĞÖ±ÈÒ»ÏÂÎä¹¦¡£");
...
if (ob->query_temp("big5") && userp(ob) && arg!="" ) arg=LANGUAGE_D->Big52GB(arg); 
        ...
        if (ob->query_temp("thd_marry") == 2) {
                ...
                if (!ob->query_temp("book5511"))
               {
               command("say ºÃĞ¡×Ó£¬ÄãÒª×öÊ²Ã´?\n");
                        return 1;
                }
                                if (!ob->query_temp("book551") && (int)ob->query("jinyong/book5")==4)
               {
               command("say ºÃĞ¡×Ó£¬Äã´òÓ®ÁËÂğ?\n");
                        return 1;
                }
...
µÀ£ººÃ°É£¬ÕâµÚ¶ş¹Ø¾ÍËãÊÇ¸öÆ½ÊÖ¡£\n", ({ob}));
		...
		who->add("mpgx",10);
		who->add("expmax",2);
		//command("chat "+this_player()->query("name")+"µ¨É«¹ıÈË£¬Òå³ÍÇà³Ç¶ñ°Ô¡£");
		tell_object (who,"ÄãÓ®µÃÁË"+chinese_number(i+14000)+"µã¾­Ñé"+
		           chinese_number(i*18)+"µãÇ±ÄÜ"+"Ê®µãÃÅÅÉ¹±Ï× ¶şµã³É³¤ÉÏÏŞ£¡£¡\n");
		write("»ÆÒ©Ê¦ËµµÀ£ºÎÒÍ½µÜÂ½¹ÚÓ¢´¦ÓĞµĞÈËÀ´ÍµÏ®£¬ÄãÓĞ¿ÕÈ¥°ï°ïËû°É¡£\n");
		who->save();
               }
